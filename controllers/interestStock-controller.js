...
export const getDetailInterestStocks = async (req, res) => {
  ...
  try {
    ...
    if (interestStock) {
      ...
      const stocks = await Stock.find({ reuters_code: { $in: reutersCodes } }).session(session);
...
export const insertInterestStock = async (req, res) => {
  ...
  try {
    ...
    // 관심 주식 추가
    await interestStock.addStock(reuters_code, { session });
...
export const deleteInterestStock = async (req, res) => {
  ...
  try {
    ...
    // 관심 주식 삭제
    await interestStock.removeStock(reuters_code, { session });
...
export const updateInterestStockOrder = async (req, res) => {
  ...
  try {
    ...
    // 모든 값이 존재하는지 확인
    for (const reuters_code of parse_stockList) {
      if (!reutersCodes.includes(reuters_code)) {
        isError = true;
        res.status(400).json({
          ok: false,
    ...
    // 주식 리스트 순서 업데이트
    if (!isError) {
      await interestStock.updateStockOrder(parse_stockList, { session });
      await session.commitTransaction();
      res.status(200).json({ ok: true, data: interestStock.stock_list ?? [] });
    }
  } catch (err) {
