...
import { STOCK_NAME_LIST, STOCK_REUTERS_CODE } from "../constants/app.constants.js";
...
export const getSearchNews = async (req, res) => {
  try {
    ...
    const searchStockList = STOCK_NAME_LIST.filter((stock) => hangulIncludes(stock, stock_name)).map(
      (stock_name) => STOCK_REUTERS_CODE[stock_name]
    );
    ...
    const searchNews = await News.find(
      ...
      { index: 1, title: 1, published_time: 1, publisher: 1, thumbnail_url: 1, _id: 0 }
    ).sort({
      published_time: -1,
      score: -1,
      view: -1,
