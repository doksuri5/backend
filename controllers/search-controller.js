...
export const getSearchStocks = async (req, res) => {
  try {
    ...
    const stockData = await Stock.find({ stock_name: { $in: searchStockList } }); // stockNames 리스트에 있는 값들만 조회
...
export const getRecentSearchDetails = async (req, res) => {
  try {
    ...
    // 최근 검색어 DB 조회
    const searches = await RecentSearch.find({ user_snsId: snsId, is_delete: false }).sort({
      search_date: -1,
    });
...
export const saveRecentSearch = async (req, res) => {
  try {
    ...
    for (const stock of searchStockList) {
      const existingSearch = await RecentSearch.findOne({ user_snsId: snsId, is_delete: false, stock_name: stock });
