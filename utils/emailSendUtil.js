...
const htmlCertContent = (VERIFICATION_CODE) => {
  return `
    <!DOCTYPE html>
    <html>
      <head>
      ...
      <body>
        <div class="email-container">
          <div class="header" style="padding: 10px; text-align: start; font-size: 24px">
            Aightnow 회원 탈퇴 인증 코드입니다.
          </div>

          <div class="content" style="margin-top: 20px; padding: 10px">
            <p>안녕하세요,</p>
            <p>Aightnow 회원 인증 절차를 진행해 주시기 바랍니다. 아래의 인증코드를 사용하여 회원 탈퇴를 진행해 주세요.</p>
            <p style="font-size: 20px; margin: 20px 0; color: #18254c"><strong>${VERIFICATION_CODE}</strong></p>
            <p>이 코드는 3분 동안 유효하며, 한 번만 사용할 수 있습니다.</p>
          </div>
...
const send_main_func = async ({ type, to, VALUE }) => {
  ...
  if (type === "code") {
    ...
    subject = "Aightnow 회원 탈퇴 인증코드 발급";
    html = htmlCertContent(VALUE);
  } else {
    subject = "Aightnow 임시 비밀번호 발급";
    html = htmlPWContent(VALUE);
  }
